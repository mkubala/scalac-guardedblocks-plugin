...
import com.softwaremill.guardedblocks.GuardedBlocksPlugin
...
abstract class GuardedBlocksPluginTestSuite(testDirPaths: String*) extends WordSpec with Matchers with BeforeAndAfterEach {

  sealed trait CompileTestResult {
    def label: String
  }

  case object Success extends CompileTestResult {
    val label = "compile without error"
  }

  case object Failure extends CompileTestResult {
    val label = "cause compile errors"
  }
  ...
  "Blocks guardian" should {
    for {
      testDirPath <- testDirPaths
      testCase <- testCasesFrom(testDirPath)
    } {
